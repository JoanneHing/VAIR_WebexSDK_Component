...
// Caption Section --------------------------------------------------
let lastSentRasaCaptionText = '';

const liveCaptionsDisplay = document.querySelector('#live-captions-display');
const lineByLineCaptionTemplate = document.querySelector('#line-by-line-caption-template');
const virtualReceptionistSpeakerId = "cc61b95a-f806-46a3-aa64-f022b45966db";
...
function setTranscriptEvents() {
  console.log('=== setTranscriptEvents() CALLED ===');
  const meeting = getCurrentMeeting();
  ...
  if (meeting) {
    ...
    var liveCaptionCompiledTemplate = Handlebars.compile(lineByLineCaptionTemplate.innerHTML);
    ...
    meeting.on('meeting:caption-received', (payload) => {
      // Check if the payload has captions
      ...
      console.log("update caption received starting");

      // Used to store the last sent caption text to Rasa
      let latestCaption = null;

      // Iterate through the captions in reverse order to find the latest final caption
      for (let i = payload.captions.length - 1; i >= 0; i--) {
        // Get the specific caption at index i
        const caption = payload.captions[i];

        // Get the speaker ID to check if it's the virtual receptionist
        const speakerId = caption.speaker ? caption.speaker.speakerId : 'N/A (no speaker)';
        console.log(`Checking caption: isFinal=${caption.isFinal}, Speaker ID=${speakerId}`);

        // Check if the caption is final AND it's NOT from the virtual receptionist speaker
        const isFromAvoidedSpeaker = caption.speaker && caption.speaker.speakerId === virtualReceptionistSpeakerId;
        console.log(`Is from avoided speaker? ${isFromAvoidedSpeaker}`);

        // If the caption is final and not from the virtual receptionist, we store it
        if (caption.isFinal && (!caption.speaker || caption.speaker.speakerId !== virtualReceptionistSpeakerId)) {
          latestCaption = caption;
          break;
        }
      }

      // Display the latest caption and send it to Rasa if it's not a duplicated text extracted from the previous captions
      // This is because while webex is extracting captions, it updates the json continuously and causes duplicate captions captured
      // By checking the text of the latest caption against the last sent text to Rasa, we avoided sending duplicates
      if (latestCaption) {
        console.log(`Latest caption found:`);
        liveCaptionsDisplay.innerHTML = liveCaptionCompiledTemplate({
          captions: [latestCaption]
        });

        // If the latest caption text is the same as the last sent text to Rasa, skip sending it
        if (latestCaption.text === lastSentRasaCaptionText) {
          console.log("Skipping sending duplicate Rasa caption:", latestCaption.text);
          return;
        }
        
        // Send the latest caption text to Rasa to process
        const senderIdForRasa = latestCaption.speaker ? latestCaption.speaker.speakerId : "webex_user_default";
        sendCaptionToRasa(latestCaption.text, senderIdForRasa);
        lastSentRasaCaptionText = latestCaption.text;
      } else {
        liveCaptionsDisplay.innerHTML = '';
      }
    });
...
const rasaDisplay = document.querySelector('#rasa-display');
const clearRasaStatusBtn = document.querySelector('#clear-rasa-status-btn');

// Send latest caption text to Rasa to process
async function sendCaptionToRasa(messageText, senderId) { 
    // The Rasa endpoint URL (This is configured in the RASA project) 
    const rasaEndpoint = "http://localhost:5005/webhooks/rest/webhook";

    if (!messageText || messageText.trim() === "") {
        console.warn("Skipping sending empty message to Rasa.");
        return;
    }

    // New entry for Rasa integration
    const rasaEntry = document.createElement('div'); // Renamed variable
    rasaEntry.classList.add('rasa-history-entry');

    // Append message sent
    const messageSentToRasa = document.createElement('p');
    messageSentToRasa.innerHTML = `<strong>Sent:</strong> "${messageText}"`;
    rasaEntry.appendChild(messageSentToRasa);

    // Placeholder for status and response
    const RasaStatus = document.createElement('p');
    RasaStatus.innerHTML = `<strong>Status:</strong> <span style="color: orange;">Sending...</span>`;
    rasaEntry.appendChild(RasaStatus);

    const RasaResponse = document.createElement('p');
    RasaResponse.innerHTML = `<strong>Rasa Response:</strong> <em>Waiting...</em>`;
    rasaEntry.appendChild(RasaResponse);

    rasaDisplay.appendChild(rasaEntry);

    rasaDisplay.scrollTop = rasaDisplay.scrollHeight;
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log('Message sent to Rasa successfully!', data);
            RasaStatus.innerHTML = `<strong>Status:</strong> <span style="color: green;">Received</span>`;

            let rasaResponses = [];
            if (Array.isArray(data)) {
                data.forEach(responseItem => {
                    if (responseItem.text) {
                        rasaResponses.push(responseItem.text);
                    }
                });
            }

            RasaResponse.innerHTML = `<strong>Rasa Response:</strong> ${rasaResponses.length > 0 ? rasaResponses.join(' | ') : 'No text response.'}`;

            // If there are responses from Rasa, send them to HeyGen
            if (rasaResponses.length > 0) {
              // Join all Rasa responses into a single string with ' | ' as separator
              const fullRasaResponse = rasaResponses.join(' | '); 
              // Split the response into parts based on ' | ' to send them one by one
              const responseParts = fullRasaResponse.split(' | '); 

              // Send each part to HeyGen with a delay to simulate the interaction flow more naturally
              // This is because in the backend of RASA, the responses may include integrations that require some time to process
              (async () => {
                  for (let i = 0; i < responseParts.length; i++) {
                      const part = responseParts[i].trim();
                      if (part) { 
                          await sendTextToHeyGen(part); 
            ...
            console.error('Rasa error response:', errorText);

            RasaStatus.innerHTML = `<strong>Status:</strong> <span style="color: red;">Not Received (Error: ${response.status})</span>`;
            RasaResponse.innerHTML = `<strong>Rasa Response:</strong> <span style="color: red;">Error: ${errorText.substring(0, Math.min(errorText.length, 100))}...</span>`;
        }
    } catch (error) {
        console.error('Error connecting to Rasa:', error);

        RasaStatus.innerHTML = `<strong>Status:</strong> <span style="color: red;">Not Received (Network Error)</span>`;
        RasaResponse.innerHTML = `<strong>Rasa Response:</strong> <span style="color: red;">Connection error: ${error.message}</span>`;
    }
}

function clearRasaStatus() {
  rasaDisplay.innerHTML = '';
}

// HeyGen Integration
// Store the session ID to be used for sending task to HeyGen
let heyGenActiveSessionId = null;
// Base endpoint for the HeyGen proxy server
const HEYGEN_PROXY_BASE_ENDPOINT = 'http://localhost:3001';

// Function to retrieve the HeyGen session ID from the proxy server
async function getHeyGenSessionId() {
    try {
        console.log('Attempting to retrieve HeyGen session list...');
        const response = await fetch(`${HEYGEN_PROXY_BASE_ENDPOINT}/get-heygen-sessions`, {
            method: 'GET',
            headers: {
        ...
        if (response.ok) {
            ...
            if (data.data && data.data.sessions && data.data.sessions.length > 0) {
                // Assuming only have one active session, take the first one
                const sessionId = data.data.sessions[0].session_id; 
                console.log(`Successfully retrieved HeyGen Session ID: ${sessionId}`);
                heyGenActiveSessionId = sessionId; // Store it 
                return sessionId;
            } else {
                console.warn('No active HeyGen sessions found or response format unexpected.');
                heyGenActiveSessionId = null;
                return null;
            }
        } else {
            console.error('Failed to retrieve HeyGen session list:', response.status, response.statusText);
            const errorText = await response.text();
            console.error('HeyGen session list proxy error response:', errorText);
            heyGenActiveSessionId = null;
            return null;
        }
    } catch (error) {
        console.error('Error connecting to HeyGen session list proxy:', error);
        heyGenActiveSessionId = null;
        return null;
    }
}

// Function to send text to HeyGen via the proxy server
async function sendTextToHeyGen(text) {
    // Ensure we have an active session ID before sending text
    if (!heyGenActiveSessionId) {
        console.warn('No active HeyGen session ID available. Attempting to fetch...');
        await getHeyGenSessionId(); // Try to fetch it if it's not set
        if (!heyGenActiveSessionId) {
            console.error('Failed to get an active HeyGen Session ID. Cannot send text.');
            return; // Exit if still no session ID
        }
    }

    if (!text || text.trim() === '') {
        console.warn('Skipping sending empty text to HeyGen.');
        return;
    }

    try {
        const response = await fetch(`${HEYGEN_PROXY_BASE_ENDPOINT}/send-to-heygen`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                session_id: heyGenActiveSessionId,
                text: text
            })
        });
        ...
        if (response.ok) {
            const data = await response.json();
            console.log('Text sent to HeyGen successfully via proxy!', data);
        } else {
            console.error('Failed to send text to HeyGen via proxy:', response.status, response.statusText);
            const errorText = await response.text();
            console.error('HeyGen proxy error response:', errorText);

            // If the session is closed, clear the ID so it's re-fetched next time
            if (errorText.includes("session state wrong: closed")) {
                console.log("HeyGen session appears closed. Clearing session ID for re-fetch.");
                heyGenActiveSessionId = null;
            }
        }
    } catch (error) {
        console.error('Error connecting to HeyGen proxy:', error);
        heyGenActiveSessionId = null; 
    }
}
...
window.onload = () => {
  ...
  if(params.get('meetings') !== null){
    meetingSdk.src = 'https://unpkg.com/webex@latest/umd/meetings.min.js';
  }
  else{
    meetingSdk.src = 'https://unpkg.com/webex@latest/umd/webex.min.js';
  }
